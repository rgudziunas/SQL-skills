...
<form action="" method="post" class="d-grid gap-3">
    ...
    <select id="fk_Filialasid_Filialas" <?php if(isset($data['editing'])) { ?> readonly="readonly" <?php } ?> name="fk_Filialasid_Filialas" class="form-control" >
        <?php 
		$coursef2Obj = new coursef2();
        $branchIds = $coursef2Obj->getBranchIds();
	...
	<h4 class="mt-3">Prideti atsiliepimus</h4>
  <div class="row">
    <div class="form-group col-md-4">
      <label for="reviewText">Atsiliepimas</label>
      <textarea id="reviewText" name="reviewText" class="form-control"></textarea>
		
    </div>

	<div class="form-group col-md-4">
    	<label for="reviewDate">Data</label>
    	<input type="date" id="reviewDate" name="reviewDate" class="form-control">
	</div>

	<div class="form-group col-md-4">
    <label for="rating">Įvertinimas (1-10)</label>
    <select id="rating" name="rating" class="elementSelector form-select form-control">
        <?php for ($i = 1; $i <= 10; $i++): ?>
    ...
    <div class="form-group col-md-4">
      <label for="clientNickname">Autorius</label>
      <select id="clientNickname" name="clientNickname" class="elementSelector form-select form-control">
        <?php
			$coursef2Obj = new coursef2();
        	 $authors = $coursef2Obj->getAuthor();
			 foreach ($authors as $key => $val) {
				 echo "<option value='{$val['Vartotojo_vardas']}'>{$val['Vartotojo_vardas']}</option>";
			}
        ?>
      </select>
    </div>
